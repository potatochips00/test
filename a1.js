...
function drawPolygon(ctx, poly)
{
  ...
  if (!poly) {
  	alert("Empty poly-array");
  }
  for (var i = 0; i < poly.length; i++) {
    // Need to check for last point of polygon, connect with first point.
  	if (i !== (poly.length - 1)) {
...
function drawCircle(ctx, cx, cy, r)
{
  ...
  /* 
  Draw the circle by dividing circle to line segment.
  Dividing the whole circle to 18 pieces.
  Each line segment is a arclenth of 20 degree.
  */
  for (var i = 0; i < 360; i+=20) { 
  	var offset_1 = [0, 0];
  	offset_1[0] = r * Math.cos(i * deg_to_rad);
  	offset_1[1] = r * Math.sin(i * deg_to_rad);
  	var offset_2 = [0, 0];
  	offset_2[0] = r * Math.cos((i+20) * deg_to_rad);
  	offset_2[1] = r * Math.sin((i+20) * deg_to_rad);

  	var center = [cx, cy];
  	var point_1 = transformPoint(center, translateByOffset(offset_1));
  	var point_2 = transformPoint(center, translateByOffset(offset_2));

  	drawLineSegment(ctx, point_1[0], point_1[1], point_2[0], point_2[1]);
  }
}
...
function rotationAboutPoint(angle, joint)
{
  ...
  T = translateByOffset(joint); // Set the translation for translate to joint.
  T_negative = translateByOffset(joint_negative); // Set the translation for translate to origin.
  var R = [ // Set the rotation transformation.
  	[Math.cos(angle), -Math.sin(angle), 0],
  	[Math.sin(angle), Math.cos(angle), 0]
  ];
 
  T = composeTransforms(composeTransforms(T_negative, R), T); // Compose all transformation together.
  return T;
}
...
function drawPenguin(ctx)
{
  ...
  /* Draw eyes and iris */
  var eyes_offset = [-27, -20]; // Set the offset for eyes and iris.
  eyes = transformPoint(eyes_offset, head_T); // Connect the eyes to head polygon
  eyes = transformPoint(eyes, torso_T); // Connect the eyes to the body

  drawCircle(ctx, eyes[0], eyes[1], eye_r); // Draw eyes

  iris = transformPoint(eyes_offset, head_T); // Use the same offset for eyes, link it to head
  iris = transformPoint(iris, torso_T); // Link the iris to whole body

  drawCircle(ctx, iris[0], iris[1], iris_r); // Draw iris

  /* Draw upper month */
  var upper_beak_offset = [-90, 0]; // Set the offset for upper month, relative to head.
  var upper_beak_T = composeTransforms(translateByOffset(upper_beak_offset), head_T); // Composed the transform relative to head.
  ...
  /* End of drawing upper month */

  /* Draw lower month */
  var lower_beak_offset = [0, mouth_gap+28]; // Set the offset for lower beak relative to upper beak.
  var lower_beak_T = composeTransforms(translateByOffset(lower_beak_offset), upper_beak_T);
  ...
  /* End of drawing lower month */

  /* Draw arms */
  var arm_offset = [18, -30]; // The location of the arm relative to the body
  var arm_joint_offset = [-3, -58]; // The location of the arm joint relative to the arm center
  ...
  /* Draw legs and legs' joints */
  var leg_offset_left = [-70, 180]; // The location of the left leg relative to the body
  var leg_joint_offset = [0, -40];  // The location of the leg joints relative to the leg center, for both left and right leg.
  var leg_left_T = composeTransforms(
  	rotationAboutPoint(hip_angles[0], leg_joint_offset),
  	translateByOffset(leg_offset_left)
  	);
  ...
  hip_joints[0] = transformPoint(leg_joint_offset, leg_left_T);
  hip_joints[0] = transformPoint(hip_joints[0], torso_T);
  drawCircle(ctx, hip_joints[0][0], hip_joints[0][1], hip_joint_r);

  var leg_offset_right = [70, 190]; // The location of the right leg relative to the body
  var leg_right_T = composeTransforms(
  	rotationAboutPoint(hip_angles[1], leg_joint_offset),
  	translateByOffset(leg_offset_right)
  	);

  var leg_poly_right = transformPolygon(leg_poly, leg_right_T);
  leg_poly_right = transformPolygon(leg_poly_right, torso_T);
  drawPolygon(ctx, leg_poly_right);

  hip_joints[1] = transformPoint(leg_joint_offset, leg_right_T);
  hip_joints[1] = transformPoint(hip_joints[1], torso_T);
  drawCircle(ctx, hip_joints[1][0], hip_joints[1][1], hip_joint_r);
  /* End of drawing legs */

  /* Draw foot and foot's joints */
  var foot_offset = [-55, 40]; // The location of the foot relative to the leg, for both left and right foot.
  var foot_joint_offset = [45, 0]; // The location of the foot joints relative to the foot center, for both left and right foot.
  var foot_left_T = composeTransforms(
  	rotationAboutPoint(ankle_angles[0], foot_joint_offset),
  	translateByOffset(foot_offset)
  	);
  foot_left_T = composeTransforms(foot_left_T, leg_left_T);
  ...
  ankle_joints[0] = transformPoint(foot_joint_offset, foot_left_T);
  ...
  foot_right_T = composeTransforms(foot_right_T, leg_right_T);

  var foot_poly_right = transformPolygon(foot_poly, foot_right_T);
  foot_poly_right = transformPolygon(foot_poly_right, torso_T);
  drawPolygon(ctx, foot_poly_right);

  ankle_joints[1] = transformPoint(foot_joint_offset, foot_right_T);
